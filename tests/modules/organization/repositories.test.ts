...
import { formatDepartmentDisplayName } from "@/modules/organization/server/user-mappers";
...
const loadUsersRepository = async () => {
  ...
  const repository = await import("@/modules/organization/server");
...
describe("user ordering", () => {
  it("lists users by information update time descending", async () => {
    ...
    expect(repository.listUsersPage(companyId, {}, { page: 1, pageSize: 2 })).toMatchObject({
      items: [
        { openId: "recent-b" },
        { openId: "recent-a" }
      ],
    ...
    expect(
      repository.listUsersPage(companyId, {}, { page: 2, pageSize: 2 }).items
...
describe("user history department display", () => {
  it("resolves department ids and marks deleted departments", async () => {
    ...
    expect(
      repository.listUserHistoryPage(companyId, "user-1", { page: 1, pageSize: 1 })
    ).toMatchObject({
      items: [{ id: event.id }],
      page: 1,
      pageSize: 1,
